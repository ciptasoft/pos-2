	...
	if(rem==0)
	{
		...
		var cashAmount =  $("#cashAmount").val();
		 ...
		 $j.ajax({
		  ...
		  data: 'bankAmount='+bankAmount+'&crediteAmount='+crediteAmount+'&cardAmount='+cardAmount+'&cashAmount='+cashAmount+'&invoiceId='+invoiceId+'&discount='+discount+'&totalAmount='+totalAmount,
		  cache: false,
		  success: function(data)
		  {
		   window.location.href="<?php echo Yii::app()->params->base_path;?>user/raiseInvoice/id/"+data;
		   $j("#loading").hide();
		   $j(".content").html(data);
...
function activeDiscount()
	{
		...
		$j('#upc_codeP').attr( "value","");
		$j('#upc_codeP').removeAttr( "disabled","disabled");
		$j("#firstColumnP").replaceWith('<td id="firstColumnP">DISCOUNT: &nbsp;&nbsp;(%) <input type="radio" value="0" name="disc" />($) <input type="radio" value="1" name="disc" /></td>');
		$j('#firstColumnP').css( "background-color","red");
	}
...
function addDiscount()
	{
		...
		$j.ajax({
		  type: 'POST',
		  url: '<?php echo Yii::app()->params->base_path;?>user/addDiscount',
		  data: 'invoiceId='+invoiceId+'&upc_code='+upc_code+'&discountType='+discountType,
		  cache: false,
		  success: function(data)
		  {
			$j("#totalRemaining").text(remaining);
			$j("#totalPayableAmount").val(total); 
			$j("#discount").val(upc_code);
			$j("#totalAmount").val(total);
...
<div class="mainContainer" style="width:303%;">
  <div class="" id="mainContainer" style="margin-left:0px; margin-right:0px;">
     <div class="thiredcont" style="padding:0 !important;">
            <div class="topbutton">
              <p><a href="#" class="first"><img src="images/file_icon.png" title="Hold Ticket" width="30" height="20" /></a> 
                <a href="#" onClick="loadBoxContent('<?php echo Yii::app()->params->base_path ; ?>user/BrowseProduct','mainContainer');">Browse</a> 
                <a href="#" onClick="searchProduct();">Search</a> 
                <a href="#">Scan</a> 
                <a href="<?php echo Yii::app()->params->base_path ; ?>user/logout">Logout</a></p>
            ...
            <div class="greenbox" id="greenbox" style="background-color:#333;">
              ...
              <table id="paymentsummary" width="100%" border="0" cellpadding="0" cellspacing="1" style="font-size:14px; color:#FFF;">
       	  ...
       	  <tr>
           	 ...
           	 <div style="height:30px; margin-top:30px; color:#FFF;" align="left" id="paybtn"><input style=" width:110px; height:43px;" id="pay" onClick="submitTicketData();" value="PAY" type="button" class="btn" /><input style=" width:110px; height:43px;" id="pay"onclick="submitTicket(<?php echo $_POST['invoiceId'] ; ?>);" value="CANCEL" type="button" class="btn" /></div>	
            </div>
            <div class="clear"></div>
            <div class="calc1">
            <form name="CalculateP">
              <table width="100%" border="0" cellpadding="0" cellspacing="1" style="font-size:14px;">
                ...
                <tr>
                  <td class="last" onClick="card();"><b style="color:#333333;">Card</b></td>
                ...
                <tr>
                  <td onClick="bank();"><b style="color:#333333;">Cheque</b></td>
                 ...
                 <tr>
                   <td onClick="credite();"><b style="color:#333333;">Credit</b></td>
