...
function getStoreByDate()
{
	var date = $j("#searchInDate").val();
	
	$j.ajax({			
	type: 'POST',
	url:  '<?php echo Yii::app()->params->base_path;?>admin/getDailyStoreSalesReport',
	data:'date='+date,
	cache: true,
	success: function(data)
	{
		$j(".content-box").html(data);
            ...
            <div id="searchByDate">
                      <table class="search-table" >
                       ...
                       <input type="button" onclick="getStoreByDate();" value="##_ADMIN_SEARCH_##"  class="btn" style="margin-left:40px;" />
                       </td>
                       </tr>
                       </table>
                      
                       </div>
            <h2><?php if(isset($date)){echo $date ;} else { echo "Today's" ; }?>  Stores Purchase Report</h2>
   		  <table cellpadding="0" cellspacing="0" border="0" class="listing" width="960">
                    ...
                    <tr>
                        ...
                        <td style="text-align:right;"><?php echo $row['totalInvoices']; ?></td>
                   ...
                   <tr style=" background-color:#cccccc;">
						...
						<?php } ?>
						<b>(sales)<?php echo $totalQuantity ; ?></b>
                        </td>
                        <td style="text-align:right;"><?php  foreach ( $data as $row ) { 
							$totalAmount +=  $row['Amount']; ?>
						<?php } ?>
                       <b>(sales) <?php echo $totalAmount ; ?></b>
                        </td>
                   </tr>
					<?php 
                    } else { ?>
